---
title: "DM_analysis_FertilizedvsNot"
author: "Kolby Grint"
date: "2022-09-28"
...
```{r}
data2 = data1 %>%
  group_by(fertilized, county) %>%
  summarise(DM_avg = mean(DM_total))

ggplot(data2, aes(x=fertilized, y= DM_avg)) +
...
```{r}
Dunn_mod = lmer(DM_total ~ fertilized + (1|rep), data = data_Dunn)
...
```{r}
Dunn_means = emmeans(Dunn_mod, ~ fertilized, contr= "pairwise", adjust= "tukey")
...
```{r}
Dane_mod = lmer(DM_total ~ fertilized + (1|rep), data = data_Dane)
...
```{r}
Dane_means = emmeans(Dane_mod, ~ fertilized, contr= "pairwise", adjust= "tukey")
...
```{r}
Door_mod = lmer(DM_total ~ fertilized + (1|rep), data = data_Door)
...
```{r}
Door_means = emmeans(Door_mod, ~ fertilized, contr= "pairwise", adjust= "tukey")
...
```{r}
Ocon_mod = lmer(DM_total ~ fertilized + (1|rep), data = data_Ocon)
...
```{r}
Ocon_means = emmeans(Ocon_mod, ~ fertilized, contr= "pairwise", adjust= "tukey")
...
```{r}
Mara_mod = lmer(DM_total ~ fertilized + (1|rep), data = data_Mara)
...
```{r}
Mara_means = emmeans(Mara_mod, ~ fertilized, contr= "pairwise", adjust= "none")

Mara_CLD = cld(Mara_means$emmean, alpha=0.1, Letters=letters, sort=FALSE, reverse=TRUE)
...
```{r}
Outa1_mod = lmer(DM_total ~ fertilized + (1|rep), data = data_Outa1)
...
```{r}
Outa1_means = emmeans(Outa1_mod, ~ fertilized, contr= "pairwise", adjust= "tukey")
...
```{r}
Outa2_mod = lmer(DM_total ~ fertilized + (1|rep), data = data_Outa2)
...
```{r}
Outa2_means = emmeans(Outa2_mod, ~ fertilized, contr= "pairwise", adjust= "tukey")
...
```{r}
library(writexl)
write_xlsx(DMAnova_Results, "DMAnova_Results.YesvsNo.xlsx")
write_xlsx(DMMeans_Results, "DMMeans_Results.YesvsNo.xlsx")
```
